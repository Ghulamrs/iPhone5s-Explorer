...
//        let message = String("Plz select the url to use")
//        self.showAlert(title: "URL", message: message)
    }
...
func selectUrlPath(act: UIAlertAction) {
    let alertController = UIAlertController(title: "Enter your url", message: nil, preferredStyle: .alert)
    let confirmAction = UIAlertAction(title: "Done", style: .default) { (_) in
        if let txtField = alertController.textFields?.first, let text = txtField.text {//
            self.userPreference.url = text
        }
    }
